...
-- Unique partial index so seed rows can use ON CONFLICT DO NOTHING
create unique index if not exists sops_vessel_ref_code_idx
  on public.sops (vessel_id, ref_code)
  where ref_code is not null;

alter table public.sops enable row level security;

drop policy if exists "vessel crew can read sops" on public.sops;
create policy "vessel crew can read sops" on public.sops
  for select using (vessel_id = public.my_vessel_id());

drop policy if exists "officers can manage sops" on public.sops;
create policy "officers can manage sops" on public.sops
  for all using (vessel_id = public.my_vessel_id() and public.am_officer());

-- ── Seed data (skipped if ref_code already exists for this vessel) ─
do $$
...
begin
  ...
  (vid, 'deck', 'Tender Ops', 'Tender Launch & Recovery — Davit System', 'DECK-001',
  ...
  on conflict (vessel_id, ref_code) where ref_code is not null do nothing;

  insert into public.sops (vessel_id, department, category, title, ref_code, content) values
  (vid, 'deck', 'Tender Ops', 'Tender — Fuel & Pre-Use Inspection', 'DECK-002',
  ...
  insert into public.sops (vessel_id, department, category, title, ref_code, content) values
  (vid, 'engine', 'Safety', 'Engine Room Entry Procedure', 'ENG-001',
  ...
  insert into public.sops (vessel_id, department, category, title, ref_code, content) values
  (vid, 'interior', 'Guest Service', 'Guest Welcome & Cabin Preparation', 'INT-001',
